...
export default function Home() {
  ...
  const [doNotUpdate, setDoNotUpdate] = useState(0);
  ...
  const load = async () => {
    ...
    async function cacheWasm() {
      await set(
        "ffmpeg-core.wasm",
        await downloadWithProgress(`${baseURL}/ffmpeg-core.wasm`)
      );
      hasUpdatedWasm = 1;
    }
    const b =
      ((await get("ffmpeg-core.wasm")) as ArrayBuffer) || null || undefined;
    if (!b) await cacheWasm();
    await ffmpeg.load({
      coreURL: await toBlobURL(`${baseURL}/ffmpeg-core.js`, "text/javascript"),
      wasmURL: await toBlobURL(
        b
          ? window.URL.createObjectURL(new Blob([b]))
    ...
    setLoaded(true);
    setIsLoading(false);
    if (hasUpdatedWasm === 0 && doNotUpdate === 0) await cacheWasm();
  };
    ...
    >
      ...
      >
        ...
        <div
          ...
          data-tip="是否在从缓存加载FFMpeg后更新"
        >
          <button
            className="btn btn-neutral" //TODO 应随状态改变颜色;强制更新wasm的btn;无wasm缓存、初次状态下不应使用
            onClick={() => {
              setDoNotUpdate(doNotUpdate === 0 ? 1 : 0);
              set("DoNotUpdate", doNotUpdate === 0 ? 1 : 0);
            }}
          >
            自动更新：已{doNotUpdate === 0 ? "启用" : "禁用"}
          </button>
        </div>
      </div>
    </main>
